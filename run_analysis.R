...
# (c) combining subject & activity data with measurement data
test <- cbind(subject_test, ytest, xtest)
train <- cbind(subject_train, ytrain, xtrain)

# (d) merging test and train data into one single set
ds <- rbind(test, train)

# double-checking the above merge (result should be TRUE)
...
# (e) labelling the columns to match feature names
...
# checking dimension of the narrow data (3+79 columns, 10299 rows)
dim(ds)
rm("cnames")
